...
from .models import AdminModel, HdbModel
    ...
    if request.method == "POST":
        if request.form.get('login') == "Login":
            ...
            if not user:
                ...
                return render_template('adminLogin.html', user=current_user, error_message1=error_message1)
            elif not check_password_hash(user.hashedPassword, password):
                error_message2 = 'Password incorrect!'
                return render_template('adminLogin.html', user=current_user, error_message2=error_message2)
            else:
...
def admin_register():
    if request.method == "POST":
        username = request.form.get('adname').upper()
        ...
        if not username:
            ...
            return render_template('adminRegister.html', user=current_user, fill_fields=fill_fields)
        elif user:
            userexists = "User already exists"
            return render_template('adminRegister.html', userexists=userexists, user=current_user)
            ...
            return render_template('adminLogin.html', success_message=success_message, user=current_user)
        else:
